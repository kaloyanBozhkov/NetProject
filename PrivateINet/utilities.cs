...
namespace PrivateINet
{
    ...
    {
        public static byte[] CreateCommandBytesForSending (int commandId, BitArray arguments){
            byte endianness = BitConverter.IsLittleEndian ? Convert.ToByte(true) : Convert.ToByte(false);
            byte[] intBytes = BitConverter.GetBytes(commandId);
            if (BitConverter.IsLittleEndian)
                Array.Reverse(intBytes);

            int tmpx = BitConverter.ToInt32(intBytes);
            byte[] argumentsToBytes = utilities.BitArrayToByteArray(arguments);
            //First is command ID of 4 bytes, Second is arguments of command of X bytes, Last is endianness type of 1 byte
            byte[] CmdFormatted = intBytes.Concat(argumentsToBytes).Append(endianness).ToArray();
            return RSA.Encrypt(RSA.publicKey, CmdFormatted);
        }
        ...
        public static List<object> BitArrayToListObject(BitArray arguments, int[] argumentLength, byte[] argumentTypes, bool performSpecialCheckForBooleanBits = false)
        {
            ...
            int tmpBitCounter = 0;
            BitArray tmpBitArray = new BitArray(argumentTypes[0] == 1 && performSpecialCheckForBooleanBits ? 8 : argumentLength[0]);
            for (int k = 0; k < arguments.Length; k++)
            {
                tmpBitArray[tmpBitCounter] = arguments[k];
                tmpBitCounter++;
                tmpArgumentCounter++;
                if ((performSpecialCheckForBooleanBits && argumentTypes[argumentCounter] == 1 && tmpArgumentCounter == 8) || ((argumentTypes[argumentCounter] != 1 || !performSpecialCheckForBooleanBits) && argumentLength[argumentCounter] == tmpArgumentCounter))
                {//if current argument's bits matches the specific argument's length found inside of the array of argument lengths
                    ...
                    {
                        argumentCounter++;
                        if (argumentTypes[argumentCounter] == 1 && performSpecialCheckForBooleanBits)
